#!/usr/bin/env node
/*
 * P1 main-path reference check.
 * Verifies that runnable HTML entries do not load legacy/root split files
 * and that deprecated root guard stubs still fail fast.
 *
...
for (const htmlRel of rootHtmlEntries) {
  const source = read(htmlRel);
  const renderDiagnosticsIdx = source.indexOf('src/presentation/render/diagnostics/render-diagnostics.js');
  const renderIdx = source.indexOf('src/presentation/render/render.js');
  if (renderIdx >= 0 && renderDiagnosticsIdx < 0) {
    errors.push(htmlRel + ': render-diagnostics.js must be loaded before presentation/render/render.js');
  } else if (renderIdx >= 0 && renderDiagnosticsIdx > renderIdx) {
    errors.push(htmlRel + ': render-diagnostics.js appears after presentation/render/render.js');
  }
}
