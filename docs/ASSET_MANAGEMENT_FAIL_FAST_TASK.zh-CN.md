...
## 目标
对旧 asset-management 通路做 fail-fast 约束，确保所有素材管理与导入相关逻辑只能通过 `src/asset-management/asset-management.js` 进入。

## 本轮实际实现
- 给关键 asset-management 导出函数加上 owner 标记。
- 新增 `assertAssetManagementOwnership(context)`，用于校验运行时关键导出是否确实属于 `src/asset-management/asset-management.js`。
- 统一 fail-fast 报错标记：`[LEGACY-ASSET-PATH-CALLED]`。
- 在 `src/app.js` 启动阶段增加 owner 校验，若发现旧通路残留或后续覆盖，立即报错。
...
## 行为说明
只要上述关键导出缺失，或者不再由 `src/asset-management/asset-management.js` 持有，程序会立即抛出包含 `[LEGACY-ASSET-PATH-CALLED]` 的错误。
