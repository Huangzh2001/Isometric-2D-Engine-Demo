...
set LOGFILE=%~dp0logs\server-self-check.log
call :pick_port
if errorlevel 1 goto :port_failed
start "Isometric Room Self Check Server" /min cmd /c ""%~dp0server\run_server.bat" %PORT% selfcheck"
call :wait_ready
if errorlevel 1 goto :server_failed
start "" "http://127.0.0.1:%PORT%/index.html?selfCheck=1&selfCheckDownload=0"
exit /b
...
echo Open logs\server-self-check.log in the project folder to see the actual error.
