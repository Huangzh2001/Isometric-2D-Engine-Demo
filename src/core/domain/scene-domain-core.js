  ...
  function projectWorldBoxes(proto, cellX, cellY, supportZ) {
    ...
    for (var i = 0; i < voxels.length; i++) {
      ...
      out.push({
        ...
        liquidType: v.liquidType != null ? String(v.liquidType) : (v.fluidType != null ? String(v.fluidType) : null),
        fluidType: v.fluidType != null ? String(v.fluidType) : (v.liquidType != null ? String(v.liquidType) : null),
        liquidDepth: v.liquidDepth != null ? Math.max(0, Math.min(1, Number(v.liquidDepth) || 0)) : (v.waterAmount != null ? Math.max(0, Math.min(1, Number(v.waterAmount) || 0)) : null),
        waterAmount: v.waterAmount != null ? Math.max(0, Math.min(1, Number(v.waterAmount) || 0)) : (v.liquidDepth != null ? Math.max(0, Math.min(1, Number(v.liquidDepth) || 0)) : null),
        fluidAmount: v.fluidAmount != null ? Math.max(0, Math.min(1, Number(v.fluidAmount) || 0)) : null,
