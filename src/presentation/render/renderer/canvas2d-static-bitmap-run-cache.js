(function () {
  ...
  function buildStaticPacketRunBitmap(deps, geometry, meta) {
    ...
    return {
      ...
      runStartIndex: Number(meta.runStartIndex != null ? meta.runStartIndex : -1),
      firstDynamicIndex: Number(meta.firstDynamicIndex != null ? meta.firstDynamicIndex : -1),
      firstPacketId: geometry.projectedPackets.length ? String(geometry.projectedPackets[0].packet && geometry.projectedPackets[0].packet.id || '') : '',
      lastPacketId: geometry.projectedPackets.length ? String(geometry.projectedPackets[geometry.projectedPackets.length - 1].packet && geometry.projectedPackets[geometry.projectedPackets.length - 1].packet.id || '') : '',
